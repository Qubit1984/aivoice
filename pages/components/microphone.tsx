...
export default function Microphone({ onVoiceChange }: any) {
  ...
  const BUFFER_TIME = 2000; // 缓冲时间（单位：毫秒）
  const textRef = useRef<string[]>([]);

  //const [timer, setTimer] = useState<NodeJS.Timeout | null>(null);
  /*   const textCache: string[] = []; // 文本缓存数组
  let textTimer: NodeJS.Timeout | null = null; // 计时器 */
    ...
    if (apiKey && "key" in apiKey) {
      ...
      connection.on(LiveTranscriptionEvents.Transcript, (data) => {
        ...
        if (final && text) {
          ...
          console.log(final, textRef.current);
        }
  ...
  return (
    <div>
      <button onClick={() => toggleMicrophone()} className="btn btn-primary ">
        <Mic className={`${micOpen ? "mic-on" : "mic-off"}`} />
        <div className="mt-20 p-6 text-xl text-white text-center">
          {caption && micOpen
            ? caption
